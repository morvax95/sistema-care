...
class Usuario_model extends CI_Model
{
    ...
    public function registrar_usuario()
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            // $obj_usuario['clave'] = md5($this->input->post('clave'), PASSWORD_BCRYPT);
            //$obj_usuario['clave'] = sha1($this->input->post('clave'), PASSWORD_BCRYPT);
            // $obj_usuario['clave'] =hash ('sha256',$this->input->post('clave'), PASSWORD_BCRYPT);
            //$obj_usuario['clave'] =hash ('sha384',$this->input->post('clave'), PASSWORD_BCRYPT);


            // $hash = hash('sha256',''); function createSalt() { $text = md5(uniqid(rand(), TRUE)); return substr($text, 0, 3); } $salt = createSalt(); $password = hash('sha256', $salt . $hash);
            ...
            //intentando colocar mensaje de error
